...
Call the filter method to filter the message. See Filter Flow for the function
definition
...
Call the transform method to transform/modify the message. See Transform Flow
...
Call the store method to persist the message to a data store. See Store Config
...
Currently only TCP (MLLP), HTTP, and HTTPS destinations are supported, but
coming soon, Gofer Engine will also support ftp, sftp, and database direct
queries/mutations.
...
You can also use a function to extract this information from the message or
context
