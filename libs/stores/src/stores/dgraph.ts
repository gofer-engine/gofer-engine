...
type StoredSubcomponent = Omit<StrictSubComponent, 'position'> & Node & {
  ['Subcomponent.position']: number
}

type StoredComponent = Omit<StrictComponent, 'value' | 'position' | 'subComponents'> & Node & {
  ['Component.position']: number
  ['Component.value']: string
  subcomponents: StoredSubcomponent[]
}

type StoredFieldRep = Omit<StrictFieldRepetition, 'value' | 'position' | 'components'> & Node & { 
  ['FieldRep.position']: number,
  ['FieldRep.value']: string
  components: StoredComponent[]
}

type StoredField = Omit<StrictField, 'value' | 'position' | 'repetitions'> & Node & {
  ['Field.position']: number
  ['Field.value']: string
  repetitions: StoredFieldRep[]
}

type StoredSegment = Omit<StrictSegment, 'value' | 'position' | 'name' | 'fields'> & Node & {
...
type StoredMessageMeta = StrictMessage['meta'] & Node & {
...
class DBStore implements IStoreClass {
  ...
  public store: StoreFunc = async (data) => {
    ...
    return new Promise<boolean>(async (res, rej) => {
      try {
        ...
        if (this.verbose) console.log('res.getMetrics: ', response.getMetrics())
        await txn.discard()
        res(true)
      } catch (err) {
        if (this.warnOnError) {
  ...
  public typeMessage = (msg: StrictMessage, id: string, hl7: string): StoredMessage => {
    return {
      id,
      ['dgraph.type']: ['Message'],
      meta: {
