  ...
  public store: StoreFunc = async (data) => {
      ...
      this.id === undefined
        ...
        : this.id === 'UUID'
          ...
          : this.id.match(/^\$[A-Z][A-Z0-9]{2}/)
            ...
            : this.id
    const database = this.database === undefined 
      ? undefined 
    ...
    return new Promise<boolean>(async (res, rej) => {
      try {
        ...
        const collection = this.client.db(database).collection(collectionName)
        const contents: Record<string, unknown> = this.normalized 
          ? data.json(true)
          : { meta: data.json()[0], msg: data.json()?.[1] }
        contents['_id'] = id
        const created = await collection.insertOne(contents)
        if (created.acknowledged) {
          if (this.verbose)
            console.log(`Created ID: ${created.insertedId} in ${database}:${collectionName}`)
          return res(true)
        } else {
          if (this.warnOnError) {
            console.warn(`Failed to create ID: "${id}" in ${database}:${collectionName}`)
            res(false)
          }
        }
      } catch (error) {
