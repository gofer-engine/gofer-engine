...
export class JSONMsg implements IJSONMsg {
  ...
  constructor(msg?: string | JSONValue, parse?: boolean) {
    if (parse) {
      if (typeof msg === 'string') {
        try {
          this._msg = JSON.parse(msg);
        } catch (err) {
          throw new Error(`Cannot parse string with JSON.parse: ${err}`);
        }
      } else {
        throw new Error(`Cannot parse ${typeof msg} with JSON.parse`)
      }
    } else {
      if (msg !== undefined) {
  ...
  set = (path?: string, value?: JSONValue): IJSONMsg => {
